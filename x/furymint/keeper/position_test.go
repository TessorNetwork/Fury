...
func TestKeeper_NewPosition(t *testing.T) {
	testData := []struct {
		...
		{
			name:   "no ufusd requested",
			owner:  testEtpOwner.String(),
			id:     testEtp.ID,
			amount: sdk.NewInt(0),
			error:  fmt.Errorf("no ufusd requested"),
		},
		{
...
func TestKeeper_RemoveFUSD(t *testing.T) {
	t.Run("Non existing ETP returns error", func(t *testing.T) {
		ctx, _, _, k := SetupTestInput()

		_, err := k.RemoveFUSD(ctx, testEtpOwner, "notExists", *testEtp.Credits)
	...
	t.Run("Existing ETP can't be modified before freeze period passes", func(t *testing.T) {
		...
		_, err := k.RemoveFUSD(ctx, testEtpOwner, testEtp.ID, *testEtp.Credits)
		require.Error(t, err)
	...
	t.Run("Existing ETP but tokens requested to burn are more than initially requested", func(t *testing.T) {
		...
		_, err := k.RemoveFUSD(ctx, testEtpOwner, testEtp.ID, burnAmountBigger)
		require.Error(t, err)
	})
	...
	t.Run("Existing ETP returns correct residual", func(t *testing.T) {
		...
		k.SetPosition(ctx, testEtp)
		baseUfusdAccount := sdk.NewCoin(types.CreditsDenom, sdk.NewInt(50))
		baseUfuryAccount := sdk.NewCoin(types.BondDenom, sdk.NewInt(0))
		_ = k.bankKeeper.MintCoins(ctx, types.ModuleName, testLiquidityPool)
		coins := sdk.NewCoins(baseUfuryAccount, baseUfusdAccount)
		_ = bk.MintCoins(ctx, types.ModuleName, coins)
		_ = bk.SendCoinsFromModuleToAccount(ctx, types.ModuleName, testEtpOwner, coins)
		//_ = k.bankKeeper.AddCoins(ctx, testEtpOwner, sdk.NewCoins(baseUfuryAccount, baseUfusdAccount))
		_, err := k.RemoveFUSD(ctx, testEtpOwner, testEtp.ID, halfCoinSub)
		require.NoError(t, err)
		require.Equal(t, baseUfusdAccount.Amount.Sub(halfCoinSub.Amount), k.bankKeeper.GetAllBalances(ctx, testEtpOwner).AmountOf(types.CreditsDenom))
