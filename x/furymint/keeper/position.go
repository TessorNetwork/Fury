...
func (k Keeper) NewPosition(ctx sdk.Context, depositor string, deposit sdk.Coins, id string) error {
	...
	ufusdRequested := deposit.AmountOf(types.CreditsDenom)
	if ufusdRequested.IsZero() {
		return fmt.Errorf("no %s requested", types.CreditsDenom)
	}

	conversionRate := k.GetConversionRate(ctx)

	uccDec := sdk.NewDecFromInt(ufusdRequested)
	ufuryAmount := uccDec.Mul(conversionRate).Ceil().TruncateInt()

	// Create ufusdEmitted token
	ufusdEmitted := sdk.NewCoin(types.CreditsDenom, ufusdRequested)
	...
	position.Credits = &ufusdEmitted

	if err := position.Validate(); err != nil {
		return fmt.Errorf("could not validate position message, %w", err)
	}

	// Send the deposit from the user to the furymint account
	...
	creditsCoins := sdk.NewCoins(ufusdEmitted)
...
// RemoveFUSD burns burnAmount to the conversion rate stored in the Position identified by id, and returns the
// resulting collateral amount to user.
func (k Keeper) RemoveFUSD(ctx sdk.Context, user sdk.AccAddress, id string, burnAmount sdk.Coin) (sdk.Int, error) {
