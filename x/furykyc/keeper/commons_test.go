...
func SetupTestInput() (sdk.Context, bankKeeper.Keeper, government.Keeper, Keeper) {
	...
	mintAcc := authTypes.NewEmptyModuleAccount(furymintTypes.ModuleName, authTypes.Minter, authTypes.Burner)
	ak.SetModuleAccount(ctx, mintAcc)

	mk := furymintKeeper.NewKeeper(cdc, keys[furymintTypes.StoreKey], keys[furymintTypes.StoreKey], bk, ak, *govk, pk.Subspace(furymintTypes.ModuleName))
	...
	k.mintKeeper.UpdateParams(ctx, validFuryMintParams)
