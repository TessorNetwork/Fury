...
if [ -z "$(ls -A $CHAIN_DIR)" ]; then
	...
	./commercionetworkd unsafe-reset-all $FYD_FLAGS
	./commercionetworkd init $NODENAME $FYD_FLAGS
...
./commercionetworkd start $FYD_FLAGS $FYD_START_FLAGS &
sleep 3 # let cnd start first before running cncli rest server
./commercionetworkd rest-server --chain-id=$CHAINID --laddr $FYCLI_LISTEN_ADDR 
