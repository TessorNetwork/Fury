...
import (
	...
	governmentTypes "github.com/tessornetwork/fury/x/government/types"
...
func testnetCmd(mbm module.BasicManager, genBalIterator banktypes.GenesisBalancesIterator) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "testnet",
		Short: "Initialize files for a Fury testnet",
...
Example:
	...
	cmd.Flags().String(flagNodeDaemonHome, "fury",
		"Home directory of the node's daemon configuration")
	cmd.Flags().String(flagNodeCLIHome, "fycli",
...
func InitTestnet(
	...
	// Setup app.toml
	fydConfig := srvconfig.DefaultConfig()
	fydConfig.MinGasPrices = minGasPrices
	fydConfig.API.Enable = true
	fydConfig.API.Swagger = true
	...
	for i := 0; i < numValidators; i++ {
		...
		srvconfig.WriteConfigFile(filepath.Join(nodeDir, "config/app.toml"), fydConfig)
	}
...
func initGenFiles(
	...
	var kycState furykycTypes.GenesisState
	cdc.MustUnmarshalJSON(appGenState[furykycTypes.ModuleName], &kycState)
	...
	membership := furykycTypes.NewMembership("black", genAccounts[0].GetAddress(), genAccounts[0].GetAddress(), time.Now().Add(time.Hour*24*365))
	kycState.Memberships = append(kycState.Memberships, &membership)
	appGenState[furykycTypes.ModuleName] = cdc.MustMarshalJSON(&kycState)
