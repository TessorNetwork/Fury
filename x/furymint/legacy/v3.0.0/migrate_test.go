...
func TestMigrate(t *testing.T) {
	type args struct {
		v220GenState v220furymint.GenesisState
	}
	tests := []struct {
		name string
		args args
		...
		{
			name: "empty",
			args: args{v220GenState: v220furymint.GenesisState{}},
			want: &types.GenesisState{},
		},
		{
			...
			args: args{
				v220GenState: v220furymint.GenesisState{
					Positions: []v220furymint.Position{
						v220furymint.Position{
							Owner:      testUser01,
							Collateral: sdk.NewInt(10),
							Credits: sdk.Coin{
								Denom:  v220furymint.BondDenom,
								Amount: sdk.NewInt(5),
							},
							CreatedAt:    cAt01,
							ID:           "some-id-01",
