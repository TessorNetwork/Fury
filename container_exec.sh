...
if [ -z "$(ls -A $CHAIN_DIR)" ]; then
	...
	./furydd unsafe-reset-all $FYD_FLAGS
	./furydd init $NODENAME $FYD_FLAGS
...
./furydd start $FYD_FLAGS $FYD_START_FLAGS &
sleep 3 # let cnd start first before running cncli rest server
./furydd rest-server --chain-id=$CHAINID --laddr $FYCLI_LISTEN_ADDR 
